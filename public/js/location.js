define("location", ["base", "tools"], function($, T) {
    ...
    function AddressSelector(options, domNode) {
        ...
        _this.init = function(options, domNode) {
            ...
            var DATA = {
                regions: {
                    ...
                    "华南": ["广东省", "广西壮族自治区", "海南省"],
                    "华中": ["湖北省", "湖南省", "河南省"],
                    "华北": ["河北省", "山西省", "北京市", "天津市", "内蒙古自治区"],
                    "西北": ["宁夏回族自治区", "新疆维吾尔自治区", "青海省", "陕西省", "甘肃省"],
            ...
            if (options.addressAreaList && options.addressAreaList.length) {
                for (var p, provinces = {}, i = 0; p = options.addressAreaList[i]; i++)
                    if (p.citys && p.citys.length) {
                        for (var c, cities = {}, j = 0; c = p.citys[j]; j++)
                            if (p.citys && p.citys.length) {
                                for (var d, districts = {}, k = 0; d = c.districts[k]; k++) districts[d.name] = 0;
                                cities[c.name] = districts
                            } else cities[c.name] = 0;
                        provinces[p.name] = cities
                    } else provinces[p.name] = 0;
                _this.DATA = {
            ...
            _this.mode = 1;
            _this.level = options.level || 3;
            _this.callback = options.callback;
            _this.eventType = options.eventType || "click";
            _this.isUserChange = -1;
            if (0 == $(_this.domNode).data("mode")) {
                _this.mode = 0;
                _this.level = 1
            }
            _this.defaultAddress = options.defaultAddress || T.cookie("_address") || CFG_DB.DEF_PCD;
            _this.curAddress = null;
            _this.tab1 = ".tab_01";
            _this.tab2 = ".tab_02";
            _this.tab3 = ".tab_03";
            ...
            _init && _init.call(_this, options)
        };
        _this.getData = function(province, city, district) {
            for (var arg, _this = this, args = [], i = 0; arg = arguments[i]; i++) arg && args.length == i && args.length < _this.level && args.push(arg);
            var _recursion = function(obj, keys) {
                var key = keys[0],
                    data = null;
                if (key) {
                    data = obj[key];
                    if (keys.length > 1) return _recursion(data, keys.slice(1))
                }
                return data;
            };
        ...
        _this.each = function(obj, callback) {
            if (obj && "object" == typeof obj && callback)
                for (var o in obj)
                    if (obj.hasOwnProperty(o) && "" !== o && "string" == typeof o && callback(o, obj[o]) === !1) break
        };
        _this.attr = function(o, n, v) {
            o && n && null != v && o.setAttribute && o.setAttribute(n, v);
        ...
        _this.getPCD = function(value) {
            var _this = this,
                address = _this.parseValue(value),
                data = _this.DATA.provinces;
            null == data[address.province] && _this.each(data, function(province, flag) {
                address.province = province;
                return !1
            });
            data = data[address.province] || {};
            null == data[address.city] && _this.each(data, function(city, flag) {
                address.city = city;
                return !1
            });
            data = data[address.city] || {};
            null == data[address.district] && _this.each(data, function(district, flag) {
    ...
    SelectAddressSelector.prototype = {
        init: function(options) {
            ...
            options = options || {};
            if (options.auto)
                for (var slt, id, slts = document.getElementsByTagName("select"), i = (slts.length, 0); slt = slts[i]; i++) {
                    id = slt.id;
                    id && /^pid\-/i.test(id) && _this.initSelect(id.replace(/^pid\-/i, ""), options.province || "", options.city || "", options.district || "")
                }
            return _this
        },
        ...
        setProvince: function(suffix, province, city, district) {
            var _this = this,
                ...
                vals = _this.parseValue(province || (hid ? hid.value : "") || "", city, district);
        ...
        setDistrict: function(suffix, district) {
            ...
            var mode = slt ? _this.attr(slt, "data-mode") : 0,
                data = _this.getData(province, city);
            slt && (slt.length = mode);
        ...
        setOptions: function(obj, data, value, mode) {
            var option = null;
            if (obj) {
                mode && obj.options.length ? obj.length = 1 : obj.length = 0;
                var count = 0;
                for (var o in data)
                    if (data.hasOwnProperty(o) && "" !== o && "string" == typeof o) {
                        option = new Option(o, o);
                        o === value && (option.selected = "selected");
    ...
    window.PCD = new SelectAddressSelector({
        auto: !0
    });
    GeoLocation.prototype = {
        init: function() {
            ...
            _this.bindEvents();
            _this.showProvince();
            _this.defaultAddress && _this.setValue(_this.defaultAddress);
            return _this
        },
        ...
        bindEvents: function() {
            ...
            $(_this.domNode).off(_this.eventType + ".location").on(_this.eventType + ".location", function(e) {
                ...
                if ("mouseover" == _this.eventType || "none" == $(this).next().css("display")) {
                ...
                if (1 == $level.data("index")) _this.showCity($(this).data("value"));
                else if (2 == $level.data("index")) _this.showDistrict($(this).data("value"));
                else if (3 == $level.data("index")) {
        ...
        showProvince: function() {
            var _this = this,
                $eleLevel = $(_this.domNode).find(_this.seclevel1).empty();
            if (_this.mode) var data = _this.DATA.provinces;
            else var data = _this.DATA.regions;
            _this.render($eleLevel, data, !0)
        },
        showCity: function(province) {
            var _this = this,
                data = _this.getData(province),
                $eleLevel = $(_this.domNode).find(_this.seclevel2).empty(),
                $tab2 = $(_this.domNode).find(_this.tab2);
        ...
        showDistrict: function(city) {
            var _this = this,
                data = _this.getData(_this.curAddress.province, city),
                $eleLevel = $(_this.domNode).find(_this.seclevel3).empty(),
                $tab3 = $(_this.domNode).find(_this.tab3);
            $(_this.domNode).find("" + _this.tab2 + " span").html(city);
            $tab3.hide();
            _this.curAddress.city = city;
            if (data && 0 != data && _this.level > 2) {
                _this.render($eleLevel, data);
                ...
                for (var o in data)
                    if (data.hasOwnProperty(o) && "" !== o && "string" == typeof o) {
                        ...
                        var o_short = short ? _this.shortProvince(o) : o,
                            clsName = num % 4 == 0 ? "sec_item last" : "sec_item";
                        $eleLevel.append('<a class="' + clsName + '" href="javascript:void(0);" data-value="' + o + '" title="' + o + '">' + o_short + "</a>").show().siblings().hide()
                    }
            } else {
                $eleLevel.append('<div class="arealist clearfix border"><div class="dt">热门省市</div><div class="dd"><a class="sec_item" href="javascript:void(0);" data-value="北京市" title="北京市">北京</a><a class="sec_item" href="javascript:void(0);" data-value="广东省" title="广东省">广东</a><a class="sec_item" href="javascript:void(0);" data-value="上海市" title="上海市">上海</a><a class="sec_item" href="javascript:void(0);" data-value="湖北省" title="湖北省">湖北</a><a class="sec_item last" href="javascript:void(0);" data-value="陕西省" title="陕西省">陕西</a></div>');
                for (var o in data) {
                    var str = "";
                    if (data.hasOwnProperty(o) && "" !== o && "string" == typeof o) {
                        for (var i = 0, l = data[o].length; l > i; i++) {
                        ...
                        $eleLevel.append('<div class="arealist clearfix"><div class="dt">' + o + '：</div><div class="dd">' + str + "</div>").show().siblings().hide()
                    }
                }
            }
        },
        setAddress: function() {
            var _this = this,
                str = _this.shortProvince(_this.curAddress.province) + (_this.level > 1 ? _this.curAddress.city : "") + (_this.level > 2 ? _this.curAddress.district : "");
            _this.curAddress.value = _this.curAddress.province + "^" + _this.curAddress.city + "^" + _this.curAddress.district;
            ...
            if (T.cookie("_address") != _this.curAddress.value && _this.curAddress.value !== $(_this.domNode).data("value")) {
                ...
                _this.callback && _this.callback(_this.curAddress.value, _this.curAddress.province, _this.curAddress.city, _this.curAddress.district);
                $(_this.domNode).trigger("valueChange.location", _this.curAddress)
            }
        },
        selectItem: function(province, city, district) {
            var _this = this;
            province && _this.addCur("" + _this.seclevel1 + " a", province);
            city && _this.addCur("" + _this.seclevel2 + " a", city);
            district && _this.addCur("" + _this.seclevel3 + " a", district)
        },
        addCur: function(ele, address) {
            var _this = this;
            $(_this.domNode).find(ele).each(function(i) {
                if ($(this).data("value") == address) {
                    _this.mode || $(_this.domNode).find("" + _this.seclevel1 + " a").removeClass("cur");
                    $(this).addClass("cur").siblings().removeClass("cur");
                    return !1
                }
            })
        ...
        setValue: function(address) {
            ...
            if (!_this.curAddress.district) {
                var city = _this.getData(_this.curAddress.province),
                    curCity = _this.curAddress.city,
                    district = "";
                for (var o in city)
                    if (city.hasOwnProperty(o) && "" !== o && "string" == typeof o && (curCity ? o === curCity : !0)) {
                ...
                if (_this.mode)
                    for (var i in district)
                        if (district.hasOwnProperty(i) && "" !== i && "string" == typeof i) {
            ...
            } else $(_this.domNode).find("" + _this.tab1 + " span").html(_this.shortProvince(_this.curAddress.province));
            _this.setAddress();
    ...
    $.fn.geoLocation = function(options) {
        ...
        this.each(function() {
            var $this = $(this),
                ...
                params = "object" == typeof options && options;
            data || $this.data("geoLocation", new GeoLocation($(this)[0], params));
            if ("object" == typeof data && "string" == typeof options && "function" == typeof data[options]) {
                internalReturn = data[options].apply(data, args);
                if (void 0 !== internalReturn) return !1
            }
        });
        return void 0 !== internalReturn ? internalReturn : this
    };
    "object" == typeof T && T.initAddress && T.initAddress();
    return PCD
});